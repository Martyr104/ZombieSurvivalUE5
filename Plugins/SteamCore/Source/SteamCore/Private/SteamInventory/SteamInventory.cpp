...
bool UInventory::GetResultItemProperty(FSteamInventoryResult Handle, int32 ItemIndex, FString PropertyName, FString& OutValue)
{
	...
	if (GetInventory())
	{
		char PropertyNameChar[256];
		FCStringAnsi::Strncpy(PropertyNameChar, TCHAR_TO_UTF8(*PropertyName), 256);

		uint32 DataSize = 0;

		GetInventory()->GetResultItemProperty(Handle, ItemIndex, PropertyNameChar, nullptr, &DataSize);

		TArray<char> DataArray;
		DataArray.SetNum(DataSize);

		bResult = GetInventory()->GetResultItemProperty(Handle, ItemIndex, PropertyNameChar, DataArray.GetData(), &DataSize);
...
bool UInventory::SetPropertyString(FSteamInventoryUpdateHandle Result, FSteamItemInstanceID ItemID, FString PropertyName, FString Value)
{
	...
	if (GetInventory())
	{
		...
		FCStringAnsi::Strncpy(PropertyNameChar, TCHAR_TO_UTF8(*PropertyName), 256);
		FCStringAnsi::Strncpy(PropertyValue, TCHAR_TO_UTF8(*Value), 1024);

		bResult = GetInventory()->SetProperty(Result, ItemID, PropertyNameChar, PropertyValue);
	}
...
bool UInventory::GetItemDefinitionProperty(FSteamItemDef ItemDef, FString PropertyName, FString& OutValue)
{
	...
	if (GetInventory())
	{
		...
		// get the size of the array
		if (GetInventory()->GetItemDefinitionProperty(ItemDef, PropertyNameChar, nullptr, &DataSize))
		{
			TArray<char> DataArray;
			DataArray.SetNum(DataSize);

			bResult = GetInventory()->GetItemDefinitionProperty(ItemDef, PropertyNameChar, DataArray.GetData(), &DataSize);
