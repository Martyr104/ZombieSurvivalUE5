...
void USteamGameServer::SetKeyValue(FString Key, FString Value)
{
	...
	{
		char KeyChar[256];
		FCStringAnsi::Strncpy(KeyChar, TCHAR_TO_UTF8(*Key), 256);
		char ValueChar[1024];
		FCStringAnsi::Strncpy(ValueChar, TCHAR_TO_UTF8(*Value), 1024);

		SteamGameServer()->SetKeyValue(KeyChar, ValueChar);
	}
}
