...
void UMatchmaking::AddRequestLobbyListStringFilter(FString KeyToMatch, FString ValueToMatch, ESteamLobbyComparison ComparisonType)
{
	...
	{
		char KeyAnsi[256];
		FCStringAnsi::Strncpy(KeyAnsi, TCHAR_TO_UTF8(*KeyToMatch), 256);
		char ValueAnsi[1024];
		FCStringAnsi::Strncpy(ValueAnsi, TCHAR_TO_UTF8(*ValueToMatch), 1024);

		SteamMatchmaking()->AddRequestLobbyListStringFilter(KeyAnsi, ValueAnsi, static_cast<ELobbyComparison>((static_cast<uint8>(ComparisonType) - 2)));
	}
}
...
bool UMatchmaking::SetLobbyData(FSteamID SteamIDLobby, FString Key, FString Value)
{
	...
	{
		char KeyAnsi[256];
		FCStringAnsi::Strncpy(KeyAnsi, TCHAR_TO_UTF8(*Key), 256);
		char ValueAnsi[1024];
		FCStringAnsi::Strncpy(ValueAnsi, TCHAR_TO_UTF8(*Value), 1024);
