...
bool UFriends::SetRichPresence(FString Key, FString Value)
{
	...
	if (SteamFriends())
	{
		char KeyChar[k_cchMaxRichPresenceKeyLength];
		FCStringAnsi::Strncpy(KeyChar, TCHAR_TO_UTF8(*Key), k_cchMaxRichPresenceKeyLength);
		char ValueChar[k_cchMaxRichPresenceValueLength];
		FCStringAnsi::Strncpy(ValueChar, TCHAR_TO_UTF8(*Value), k_cchMaxRichPresenceValueLength);

		bResult = SteamFriends()->SetRichPresence(KeyChar, ValueChar);
	}
